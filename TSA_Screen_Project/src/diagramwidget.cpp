 ...
 * 
 * Creates a gray cross-hatched area below the sensor beam line
 * to visually indicate the inactive sensor coverage zone.
 * 
 * @param p QPainter reference for drawing
 */
void TSAWidget::drawHatchedArea(QPainter &p)
{
    // Build polygon from sensor line to bottom of widget
    QPolygonF poly;
    poly << sensor_line_start
         << sensor_line_end
         << QPointF(width(), height())
         << QPointF(0, height());

    // Apply gray cross-hatch brush with transparency
